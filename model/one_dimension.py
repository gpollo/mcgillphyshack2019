...
class OneDimensionalModelWrapper(QObject):
    def __init__(self):
        n = 1
        a = 1
        m = 1
    ...
    def draw(self, surface, time):
        ...
        for i in range(self.__num_cells):
            for (x, y) in self.__points:
                d = self.__system.compute_displacement(
                    self.__modes,
                    self.__w_b,
                    y, x, time
                )
            ...
            x = int(start + spacing * i)
            y = int(middle)
            r = int(7)
            c = int(0)
            pygame.gfxdraw.aacircle(surface, x, y, r, (c, c, c))
            pygame.gfxdraw.filled_circle(surface, x, y, r, (c, c, c))
